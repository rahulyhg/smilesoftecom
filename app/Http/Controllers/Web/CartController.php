...
class CartController extends DataController
{
    ...
    public function updatesinglecart(Request $request)
    {
        ...
        $product_record = DB::table('products')->where(['products_id'=> $products_id])->first();
        ...
        $final_discount = ($product_record->is_prime == 1) ? $request->products_price * $product_record->prime_percent / 100 : '0';
        //price is not default
        $final_price = $request->products_price - $final_discount;
...
//       $getPrimePercent = DB::table('alert_settings')->first();
        $product_record = DB::table('products')->where(['products_id'=> $products_id])->first();
        //price is not default
        if (!empty($request->products_price)) {
            ...
            $final_discount = ($product_record->is_prime == 1) ? $detail['product_data'][0]->discount_price * $product_record->prime_percent / 100 : '0';
            $final_price = $detail['product_data'][0]->discount_price - $final_discount;
        } else {
            $final_discount = ($product_record->is_prime == 1) ? $detail['product_data'][0]->products_price * $product_record->prime_percent / 100 : '0';
            $final_price = $detail['product_data'][0]->products_price - $final_discount;
        }
