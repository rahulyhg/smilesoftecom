...
class AdminCategoriesController extends Controller
{
    ...
    public function subcategoriesproduct()
    {
        if (session('categories_view') == 0) {
            ...
            $title = array('pageTitle' => 'Sub Category Product');
            ...
            $listingSubCategories = DB::table('categories as subCategories')
                ...
                ->select(
                    ...
                    'mainCategoryDesc.categories_name as mainCategoryName',
                    'subCategoryDesc.language_id',
                    'subCategories.categories_slug as slug'
                )
                ->where('subCategories.sabcategory_id', '>', '0')
                ->where('subCategoryDesc.language_id', '1')
                ->where('mainCategoryDesc.language_id', '1')
                ->orderBy('subId', 'ASC')->paginate(20);

            $catmanage = App\CategoryManageModel::where(['is_del'=>0])
//                ->orderBy('ASC')
                ->get();

            return view("admin.subcategory_product.subcategory_product", $title)
                ->with(['listingSubCategories'=> $listingSubCategories, 'catmanage'=>$catmanage ]);
        }
    }
    ...
    public function addsubcategoryproduct(Request $request)
    {
        $title = array('pageTitle' => Lang::get("labels.AddSubCategories"));
        $result = array();
        $result['message'] = array();
        ...
        return view("admin.subcategory_product.addsubcategory_product", $title)->with('result', $result);
    }
    ...
    public function addnewsubcategoryproduct(Request $request)
    {
        if (session('categories_create') == 0) {
            print Lang::get("labels.You do not have to access this route");
        } else {
            $cat_Manage = new App\CategoryManageModel();

            $title = array('pageTitle' => 'Add Sub Category Product');
            $date_added = date('y-m-d h:i:s');
            $result = array();
            ...
            $categoryName = $request->categoryName;
            $parent_id = $request->parent_id;
            $sabcategory_id = $request->sabcategory_id;
            ...
            $cat_Manage->icon = $uploadIcon;

            $categories_id = DB::table('categories')->insertGetId([
                'categories_image' => $uploadImage,
                'date_added' => $date_added,
                'parent_id' => $parent_id,
                'sabcategory_id' => $sabcategory_id,
                'categories_icon' => $uploadIcon
            ]);
            ...
            foreach ($languages as $languages_data) {
                ...
                $cat_Manage->category_id = $parent_id;
                $cat_Manage->sub_category_id = $sabcategory_id;
                $cat_Manage->name = request($categoryName);
                $cat_Manage->save();
            }
            ...
            $message = 'Sub category product has been added!';
    ...
    {
        $category_id = request('category_id');  //Catgeory ID Received
        $stringSelect = null;
...
//        $stringSelect = '<select class="form-control typeDD requireDD" name="parent_id" id="parent_id" onchange="category_change();">';
//        $stringSelect += '<option value="0">Select Category</option>';
//        $stringSelect += '<option value="$subcat->mainId">$subcat->mainName</option>';
//        $stringSelect +='</select>';
