...
//						}
                      ...
                      if(!empty($products->flash_price)){
                          ...
                          if($products->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                              $prime_percent = $products->prime_percent;
                              if(($prime_percent+0)>0){
                                  $discount_percentage += $prime_percent;
                              }else{
                                  $discount_percentage = 0;
                              }
                          }
                          echo "<span class='discount-tag'>".(int)$discount_percentage."%</span>";
                      }elseif($products->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                          $prime_percent = $products->prime_percent;
                          if(($prime_percent+0)>0){
                              $discount_percentage = $prime_percent;
                          }else{
                    ...
                    <div class="price text-center">                     
                        ...
                        @if(!empty($products->flash_price))
                            @if($products->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                @php
                                    $discounted_price =  $products->products_price * $products->prime_percent /100;
                                @endphp
                                {{$web_setting[19]->value}}  {{$products->flash_price-$discounted_price}}
                                <span>{{$web_setting[19]->value}}{{$products->products_price+0}}</span>
                            @else
                                {{$web_setting[19]->value}}  {{$products->flash_price+0}}
                                <span>{{$web_setting[19]->value}}{{$products->products_price+0}}</span>
                        ...
                        @else
                            @if($products->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                @php
                                    $discounted_price =  $products->products_price * $products->prime_percent /100;
                                @endphp
                                {{$web_setting[19]->value}}  {{$products->products_price-$discounted_price}}
                                <span>{{$web_setting[19]->value}}{{$products->products_price+0}}</span>
                            @else
...
//                                }
                              if(!empty($top_seller->discount_price)){
                                  ...
                                  if($top_seller->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                                      $prime_percent = $top_seller->prime_percent;
                                      if(($prime_percent+0)>0){
                                          $discount_percentage += $prime_percent;
                                      }else{
                                          $discount_percentage = 0;
                                  ...
                                  echo "<span class='discount-tag'>".(int)$discount_percentage."%</span>";
                              }elseif($top_seller->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                                  $prime_percent = $top_seller->prime_percent;
                                  if(($prime_percent+0)>0){
                                      $discount_percentage = $prime_percent;
                                  }else{
                          ...
                          {{----}}
                          	{{--@endif--}}
                                ...
                                @if(!empty($top_seller->discount_price))
                                    @if($top_seller->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                        @php
                                            $discounted_price =  $top_seller->products_price * $top_seller->prime_percent /100;
                                        @endphp
                                        {{$web_setting[19]->value}}  {{$top_seller->discount_price-$discounted_price}}
                                        <span>{{$web_setting[19]->value}}{{$top_seller->products_price+0}}</span>
                                    @else
                                        {{$web_setting[19]->value}}  {{$top_seller->discount_price+0}}
                                        <span>{{$web_setting[19]->value}}{{$top_seller->products_price+0}}</span>
                                ...
                                @else
                                    @if($top_seller->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                        @php
                                            $discounted_price =  $top_seller->products_price * $top_seller->prime_percent /100;
                                        @endphp
                                        {{$web_setting[19]->value}}  {{$top_seller->products_price-$discounted_price}}
                                        <span>{{$web_setting[19]->value}}{{$top_seller->products_price+0}}</span>
                                    @else
                                        {{$web_setting[19]->value}}  {{$top_seller->products_price+0}}
                                    @endif
...
//						}
                      ...
                      if(!empty($special->discount_price)){
                          ...
                          if($special->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                              $prime_percent = $special->prime_percent;
                              if(($prime_percent+0)>0){
                                  $discount_percentage += $prime_percent;
                              }else{
                                  $discount_percentage = 0;
                          ...
                          echo "<span class='discount-tag'>".(int)$discount_percentage."%</span>";
                      }elseif($special->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                          $prime_percent = $special->prime_percent;
                          if(($prime_percent+0)>0){
                              $discount_percentage = $prime_percent;
                          }else{
                    ...
                    <div class="price text-center">                     
                        ...
                        @if(!empty($special->discount_price))
                            @if($special->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                @php
                                    $discounted_price =  $special->products_price * $special->prime_percent /100;
                                @endphp
                                {{$web_setting[19]->value}}  {{$special->discount_price-$discounted_price}}
                                <span>{{$web_setting[19]->value}}{{$special->products_price+0}}</span>
                            @else
                                {{$web_setting[19]->value}}  {{$special->discount_price+0}}
                                <span>{{$web_setting[19]->value}}{{$special->products_price+0}}</span>
                        ...
                        @else
                            @if($special->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                @php
                                    $discounted_price =  $special->products_price * $special->prime_percent /100;
                                @endphp
                                {{$web_setting[19]->value}}  {{$special->products_price-$discounted_price}}
                                <span>{{$web_setting[19]->value}}{{$special->products_price+0}}</span>
                            @else
...
//						}
                      if(!empty($most_liked->discount_price)){
                          ...
                          if($most_liked->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                              $prime_percent = $most_liked->prime_percent;
                              if(($prime_percent+0)>0){
                                  $discount_percentage += $prime_percent;
                              }else{
                                  $discount_percentage = 0;
                          ...
                          echo "<span class='discount-tag'>".(int)$discount_percentage."%</span>";
                      }elseif($most_liked->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1){
                          $prime_percent = $most_liked->prime_percent;
                          if(($prime_percent+0)>0){
                              $discount_percentage = $prime_percent;
                          }else{
                    ...
                    <div class="price text-center">
                      ...
                      {{--@endif--}}
                          @if(!empty($most_liked->discount_price))
                              @if($most_liked->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                  @php
                                      $discounted_price =  $most_liked->products_price * $most_liked->prime_percent /100;
                                  @endphp
                                  {{$web_setting[19]->value}}  {{$most_liked->discount_price-$discounted_price}}
                                  <span>{{$web_setting[19]->value}}{{$most_liked->products_price+0}}</span>
                              @else
                                  {{$web_setting[19]->value}}  {{$most_liked->discount_price+0}}
                                  <span>{{$web_setting[19]->value}}{{$most_liked->products_price+0}}</span>
                              @endif
                          @else
                              @if($most_liked->is_prime == 1 && auth()->guard('customer')->check() && auth()->guard('customer')->user()->customers_prime == 1)
                                  @php
                                      $discounted_price =  $most_liked->products_price * $most_liked->prime_percent /100;
                                  @endphp
                                  {{$web_setting[19]->value}}  {{$most_liked->products_price-$discounted_price}}
                                  <span>{{$web_setting[19]->value}}{{$most_liked->products_price+0}}</span>
                              @else
                                  {{$web_setting[19]->value}}  {{$most_liked->products_price+0}}
                              @endif
